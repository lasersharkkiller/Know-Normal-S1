function Get-DstIPsforEnterprise{
...
$query = "endpoint.os = '$os' and src.process.name = '$srcProcName' and not(endpoint.name = '$hostName' or dst.ip.address matches '^(10\\.|127.0.0.1|192.168.|172.|169.254)') | columns dst.ip.address  | group ipCount = count (dst.ip.address) by dst.ip.address  | sort -ipCount | limit 1000"
...
if ($parentProcResponse -ne $null -and $parentProcResponse.data.queryId) {
    ...
    Write-Output "Dst IP Query (for the enterprise) created successfully with Query ID: $queryId"
} else {
...
while ($status -ne 'FINISHED') {
    try {
        ...
        Write-Host -ForegroundColor red "Could not get dst ips for the enterprise (surrounding the process from the alert). S1 API Issues. Trying again."
        $parentProcResponse = Invoke-RestMethod -Uri $queryCreateUrl -Method Post -Headers $headers -Body $params
...
if ($status -eq 'FINISHED') {
    ...
    $statusResponse.data.data | ConvertTo-Json | Out-File "output\DstIPsforEnterprise.json"
} else {
