function Get-NetworkPortsforAlertProc{
...
if ($parentProcResponse -ne $null -and $parentProcResponse.data.queryId) {
    ...
    Write-Output "Network Port Query (for the alert) created successfully with Query ID: $queryId"
} else {
...
while ($status -ne 'FINISHED') {
    try {
        ...
        Write-Host -ForegroundColor red "Could not get ports for the src proc alert. S1 API Issues. Trying again."
        $parentProcResponse = Invoke-RestMethod -Uri $queryCreateUrl -Method Post -Headers $headers -Body $params
...
if ($status -eq 'FINISHED') {
    ...
    $statusResponse.data.data | ConvertTo-Json | Out-File "output\NetworkPortsforAlertProc.json"
} else {
