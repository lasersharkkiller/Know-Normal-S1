function Get-NetworkPortsforEnterprise{
...
$query = "endpoint.os = '$os' and src.process.name = '$srcProcName' and not(endpoint.name = '$hostName') | columns dst.port.number  | group PortCount = count (dst.port.number) by dst.port.number  | sort -PortCount | limit 1000"
...
if ($parentProcResponse -ne $null -and $parentProcResponse.data.queryId) {
    ...
    Write-Output "Network Port Query (for the enterprise) created successfully with Query ID: $queryId"
} else {
...
while ($status -ne 'FINISHED') {
    try {
        ...
        Write-Host -ForegroundColor red "Could not get ports for the enterprise (surrounding the process from the alert). S1 API Issues. Trying again."
        $parentProcResponse = Invoke-RestMethod -Uri $queryCreateUrl -Method Post -Headers $headers -Body $params
...
if ($status -eq 'FINISHED') {
    ...
    $statusResponse.data.data | ConvertTo-Json | Out-File "output\NetworkPortsforEnterprise.json"
} else {
